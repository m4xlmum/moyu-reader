...
- 主题管的是**起始页这一页**（1.5.1 起的边界）：栏目线、条目、报头、状态行跟着换。顶栏、地址栏、标签条、右栏、悬浮球、弹出面板、系统设置页与 PDF 阅读页一律是纸白那套皮，换主题一个像素都不动。网页永远不受影响。
...
- **三级墨** (`#666d79`, `--text-tertiary` / `--moyu-text-faint`)：其余五栏的栏目名、动词列、右端域名、状态行、注脚。这一档量的是**页底**而不是界面的面：压页底 4.99:1，压白色的面 5.21:1（探针 `spike/theme-chrome.js` 逐条量过，`#767e8c` 那一档正是压页底不合格被否掉的）。
...
三套主题的名字沿用上一版，值全部重做。一个主题 = 一段变量组（`themes.css`），不写任何选择器；颜色与形状的对照表在 `@shared/constants` 的 `HOME_THEMES` 里。下表逐值给出三套主题；**纸白那一列与 frontmatter 是同一份值**（它是默认主题，也是兜底：`:root` 一并列出它，配置还没读回来时不会出现「变量没有值」）。

**谁把主题名写上去（1.5.1 起的边界）。** 五份文档（`index` / `home` / `popover` / `settings` / `pdf`）各自 `<link>` 同一份 `themes.css`，但**只有起始页那一份**把主题名与世界名写在 `html[data-theme]` / `html[data-world]` 上（`composables/useTheme.ts`，唯一调用点是 `HomeApp.vue`）。其余四份文档从头到尾不碰这两个属性，于是永远落在 `:root` 那一组——纸白。**没有继承路径**：一份文档的根属性传不到另一份文档的根上去，这就是这条边界得以成立的机制，也是它唯一需要的东西。

一句话说清为什么值得这么做：顶栏与右栏是**工具**，工具的样子不该因为一页换了皮就跟着变；PDF 那一页画的是**内容**，它的墨色跟着主题走的话，磷绿下整本书的字都会变成荧光绿（1.5.0 实装就是这样，1.5.1 修掉了）。因此下面两张表里**暗夜与磷绿那两列只在起始页生效**，`--moyu-*` 那张表的两列目前没有任何一处界面读得到——值留着（主题层的值只有一份，删了就成两处真相），但它们是备而不用。
...
三级墨压各自的底，探针实测（`theme-chrome.js` 的 Q6 三套一起打出来）：纸白 4.99 / 暗夜 5.0 / 磷绿 5.17（终端的整套是 13.47 / 8.81 / 5.17，页底近黑所以对比更高）。字压底一律 ≥4.5:1。

**界面（顶栏、地址栏、标签条、右栏、悬浮球、面板、系统设置页）——**1.5.1 起固定取纸白那一列

界面的「面」在主题层里存的是三通道（`255 255 255` 这种写法，不带 alpha），由 `tokens.css` 拼上「界面底板透明度」`--moyu-alpha`——只有这样那条滑块才能把底板整体乘淡，字与图标始终不透明。下表给的是等价实色（alpha = 1）。而**阅读透明度一条令牌都不碰**：它落的是**正在读的那一份**，两处都不是主题画的任何一块面——本机 TXT 是那一页整层的 `opacity`，PDF 是那张画布的**元素底色**（纸）。因此它既不在 `tokens.css` 里，也不随主题变。
...
磷绿那三个三通道面不能直接搬页上那两个 `rgba`：它们是 0.08 与 0.15 的荧光绿叠在 `#050b07` 上之后的结果（`#0c1d12` / `#112d1b`），与起始页那两个悬停**严格同色**。界面与页共用一份主题层的理由是硬的：五份文档各自加载自己的样式表，**没有继承路径**，一份文档的变量传不到另一份去；凡是跨文档要用的东西，都得由每一份文档自己写在根上（`--moyu-alpha` 早就是这么做的，见 `useBackgroundAlpha`）。主题走的也是这条路，区别只有一处：**写主题名那一行只存在于起始页那一份文档里**——这就是 1.5.1 的边界，机制与「变量不继承」是同一个事实的两面。

**网页永远不受影响。** 访客页面是另一个 `WebContentsView`，拿不到这份样式表，也拿不到这两个属性。读懂这一条就明白「主题只管起始页」的边界在哪——顺带也明白为什么这条边界只有一条实现路径：**不写属性**就等于退出主题，没有第二处开关可漏。
...
**Label/Mono Font:** 终端世界把整页换成等宽堆栈（1.5.1 起只管起始页）—— Cascadia Mono / Consolas / Sarasa Mono SC / Microsoft YaHei / monospace
...
- **透明度滑块（界面）：** 三条，待在右栏最下：整体透明度（含网页）、背景透明度（只影响界面底板）、阅读透明度（只作用在正在读的那一份上）。轨道长 **56px**、厚 14px，圆点 14px 用强调色。横条转 −90° 放置（Chromium 的 range 只有横向稳定），旋转不改变布局盒，因此外框按**旋转后**的尺寸留位（16×56），并用明确的半尺寸负外边距居中——「槽比条窄」时 grid 的 `place-items: center` 与 `inset: 0; margin: auto` 都解成「向右溢出」，转 90° 之后整条会落到可视列之外，看上去就是这个控件根本不存在。第三条比前两条短一档：前两条的下限是 5%（界面）与 0%（只淡底板，栏与控件还在），**阅读透明度也取 0**——而这个 0 在两种本机文件上落的地方不一样。**PDF 落的是那张纸**：这一页的纸是我们自己画进去的（键控那一趟把纸写成了 alpha 0、只留墨），于是这条滑块给它补一层元素底色，**值 v 对应纸的 alpha = 1 − v**——100% 是这一版之前的样子（纸全透），0% 是一张实心白纸垫在字下面，而**字始终是实心的**（滑块碰得到元素底色，碰不到位图里的墨）。**TXT 落的是正文本身**：那一页的底由 Chromium 自己画，我们碰不到（注入一张带 alpha 的用户样式表撤不回来，见 [docs/spike-findings.md](docs/spike-findings.md) 的 Q58），只能把整页 `opacity` 调低。于是同一个读数在两边读作两句话：PDF 上「透明度」字面成立（100% 就是纸全透），TXT 上仍是「正文的透明度」。默认值 1 在两处都是「什么都不加」，**老配置一个数都不用迁**。它只在读一本本机文件时才是活的：停在网页上时按右栏缩放那三格的规矩**真的禁用**（灰点、读数照旧、鼠标不是手），不装作能点。三条一起进来之后，960×540 这一档的功能栈正好放满（实测溢出 0px，迷你档 480×270 溢出 264px，由整条栈自己滚）。
...
- **只有墨，没有纸。** pdf.js 先把整页画进一张画布，再逐像素把纸收掉（键成 alpha 0，不是涂成白），剩下的字上成一份墨。因此桌面上浮着的是一层字，不是一张白纸——白纸在这个程序里等于一扇不透光的窗。1.5.2 起这条多了一句：**纸既然已经不在位图里，就随时可以当作一层元素底色补回来**——右栏那条阅读透明度补的就是它（见下）。
- **墨色是 `--moyu-ink`，而这一页读到的那一份恒是纸白的近黑。** 它不写主题（见 Themes 一节），因此永远落在 `:root` 那一组上：同一本书在哪个主题下都是近黑的字。这是 1.5.1 改的——在那之前这一页跟着主题换墨，磷绿下整本书的字都是荧光绿。理由与「网页永远不参与」是同一条：**这一页画的是内容，内容不该被界面的皮肤染上颜色**。1.5.0 那一版把它当成「唯一一处主题改变的是内容的地方」，那是想错了。深浅两态的墨都拉到量程的头（最实 253–255/255），否则桌面上会从笔画里透出一层 6.7% 的纱。
...
- **阅读透明度在这条路上落的是那张纸。** 右栏第三条滑块给画布补一层**元素底色**（`rgba(纸白, 1 − 值)`，纸白那一组三通道与墨一样从 `--moyu-surface-rgb` 读，理由同墨色那一条），**不是**给画布乘 `opacity`（1.5.2 改的）——因为这一页的位图里只剩墨，`opacity` 淡的会是字，而用户要的是纸。于是「界面 100% + 正文 40%」是成立的一种搭配，两条各走各的，且**字在这一条上永远不动**：滑块碰得到元素底色，碰不到位图里的墨（这一条与 `getImageData` 的逐字节相同一起验过，见 [docs/spike-findings.md](docs/spike-findings.md) 的 Q62）。**右下角那条浮层也不跟着动**：它是控件，不是纸（它自己那点 `opacity` 只用来做淡出，与这条滑块无关）。这是它与 TXT 那一页的分别——那一页整层 `html` 一起淡，因为它身上没有我们的控件，而它的纸是 Chromium 画的、我们碰不到。
...
5. **有一处颜色不在主题层里。** 悬浮球与裁剪弹窗的阴影是硬编码的中性黑（`rgba(17, 24, 39, …)` 与 `rgba(9, 11, 14, …)`），没有令牌支撑；其中 `#111827` 正好是上一版那套配色里的墨色。这两样都长在界面上，而界面 1.5.1 起恒为纸白，因此它一直站得住；但它仍是这套系统里唯一一处「值不在主题层」的颜色。
