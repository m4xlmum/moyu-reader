...
界面的「面」在主题层里存的是三通道（`255 255 255` 这种写法，不带 alpha），由 `tokens.css` 拼上「界面底板透明度」`--moyu-alpha`——只有这样那条滑块才能把底板整体乘淡，字与图标始终不透明。下表给的是等价实色（alpha = 1）。而**阅读透明度一条令牌都不碰**：它淡的不是主题画的任何一块面，而是被读的那份内容（本机 TXT 那一层的 `opacity`、PDF 那张画布的 `opacity`），因此它既不在 `tokens.css` 里，也不随主题变。
...
- **地址栏（界面）：** 24px 高、6px 圆角，静止时是一枚与底色同色的底、字次级墨；悬停加深，获得焦点时换成纯白 + 1px 强调色描边 + 2px 淡环。顶栏上那枚 26px 的**地址栏开关**是同一件事的另一半：一个放大镜加一个域名，`0 12px`、13px 圆角、`--moyu-surface-hover` 底；展开时变 `--moyu-accent-soft` 底 + 强调色字。它写的是**一张网页**，因此停在起始页 / 设置上时**不跟着变成那一屏的名字**——顶栏读起来会像多了一个叫「系统设置」的标签页；那两屏上它写的是刚才那张网页（也就是左上角那两颗键再点一次会回到的那一张），切进切出那一格一动不动，一张网页都没有时才一个字都不写。「此刻在哪一屏」由那两颗键各自的高亮说，不由这一格说。窗口窄到 620px 以下时它收成一枚图标——它占的 120px 是顶栏里最奢侈的一笔，标签条拿这块地方才放得下一个标签。
- **透明度滑块（界面）：** 三条，待在右栏最下：整体透明度（含网页）、背景透明度（只影响界面底板）、阅读透明度（只影响正在读的正文）。轨道长 **56px**、厚 14px，圆点 14px 用强调色。横条转 −90° 放置（Chromium 的 range 只有横向稳定），旋转不改变布局盒，因此外框按**旋转后**的尺寸留位（16×56），并用明确的半尺寸负外边距居中——「槽比条窄」时 grid 的 `place-items: center` 与 `inset: 0; margin: auto` 都解成「向右溢出」，转 90° 之后整条会落到可视列之外，看上去就是这个控件根本不存在。第三条比前两条短一档：前两条的下限是 5%（界面）与 0%（只淡底板，栏与控件还在），**阅读透明度也取 0**——它淡的是被读的东西，而右栏那三条滑块与它无关，拉到 0 剩下的是「正文不在、界面还在」。它只在读一本本机文件时才是活的：停在网页上时按右栏缩放那三格的规矩**真的禁用**（灰点、读数照旧、鼠标不是手），不装作能点。三条一起进来之后，960×540 这一档的功能栈正好放满（实测溢出 0px，迷你档 480×270 溢出 264px，由整条栈自己滚）。
- **标签条（界面）：** 每格 26px 高、6px 圆角，下限 84px、上限 180px，当前那一格用 `--moyu-surface-active` 底 + 主墨（与地址栏开关的「展开中」同一套说法）。放不下**全部**标签时整条让位，换成一枚与地址栏开关同形状的 13px 胶囊，而这一枚**分了两处**：身子写着这一条代表的那张网页，按它**进到那张网页里**；右端那枚 16px 的箭头（左侧一道 14px 高的分隔线，与身子分得开）才展开清单。两处必须各是一枚真按钮才分得开这一下，而按钮里不许再套按钮，因此箭头是 `<button>`、身子是一个 `role="button"` 的盒子（同一笔账在标签格那枚 ✕ 上算过一次，见 [docs/spike-findings.md](docs/spike-findings.md) 的 Q50）。停在起始页 / 系统设置上时标签格看不见，这一枚就是**回到网页的唯一入口**——按一下就该回去，何况它写的仍是刚才那张网页（与地址栏开关同一条规矩），切进切出那两屏时一个字都不变。
...
这一页读的是别人排好的版，因此设计能做的只有一件事：**决定什么留下来**。留的是字，纸与颜色都交出去。

- **只有墨，没有纸。** pdf.js 先把整页画进一张画布，再逐像素把纸收掉（键成 alpha 0，不是涂成白），剩下的字上成主题的墨色。因此桌面上浮着的是一层字，不是一张白纸——白纸在这个程序里等于一扇不透光的窗。
- **墨色是 `--moyu-ink`**，也就是这一页是**唯一一处主题改变的是内容而不只是外壳**的地方：同一本书纸白下是近黑的字、暗夜下是近白的字。网页永远不参与（那是硬边界），这一页参与，因为它画的东西本来就是自己的。深浅两态的墨都拉到量程的头（最实 253–255/255），否则桌面上会从笔画里透出一层 6.7% 的纱。
- **代价明写：彩色被放弃。** 插图与彩色标题按**深浅**变成一块墨。这不是「还没做」，是换「任何主题下都读得下去」付的价，README 里也这么写着。
- **右下角一条浮层。** 26px 的胶囊（`999px` 圆角）、`--moyu-surface-active` 底、`--moyu-text-dim` 的字、12px，里头是 `第几页 / 共几页`（`tabular-nums`，翻页时数字不跳宽度）与一枚 20px 的键：**自动 / 浅底 / 深底**循环，悬停落到 `--moyu-accent-soft` + `--moyu-accent`。停下 2.6 秒（`HUD_IDLE_MS`）自己淡出，而淡出时连点击一起交给正文（`pointer-events: none`）——一片看不见的按钮比没有按钮更坏。整条浮层的每一处尺寸都用 CSS 除法按 `var(--moyu-zoom)` 除回去，于是放大之后它还是同一副大小；不用 `transform: scale()` 反着缩，那会把圆角与 1px 的边一起拉花。
- **正文透明度也落在这里。** 右栏第三条滑块走到这条路上是给**画布**乘一个 opacity——不是给 `html`（那一页是自家排的，纸与墨都是我们画的，能碰的正是画布那一片）。因此「界面 100% + 正文 40%」是成立的一种搭配，两条各走各的。**右下角那条浮层不跟着淡**：它是控件，不是正文（它自己那点 `opacity` 只用来做淡出，与这条滑块无关）。这是它与 TXT 那一页唯一的分别——那一页整层 `html` 一起淡，因为它身上没有我们的控件。
- **放大是重新排版。** 右栏那条缩放改的是 dpr，画布的 CSS 宽照旧铺满正文区（`--moyu-zoom` 就是那个倍数），字重新排一遍而不是把画面拉大——拉大的图在 1.2 倍就开始糊。正文区上下留 10px、**左右留 0**：fit-width 拿 `clientWidth` 当页宽，而它含内边距。
