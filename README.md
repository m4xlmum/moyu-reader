...
当前是 **1.3.1**：里程碑 1（核心摸鱼能力）已完成并通过真机验证。1.3.0 把起始页
重做成一张按栏目分节的报纸页，并让本机的 TXT 与 PDF 直接读得起来；1.3.1 修掉起始页上
「点了没反应」那一个——界面层让回去时没有真的落回最底下，压住了正文区。
...
- **全程手动，绝不自动下载、绝不自动重启。** 安装包 110MB 以上，不问你一声就下是不礼貌的；
  重启更会打断你手上的事。点了「下载」才有进度，下完才给「重启并安装」。
- **免安装版（zip）请回发布页手动覆盖。** 它没有安装目录可更新，点「重启并安装」会把新版本
  装成**第二份**（那时删掉旧的那份目录即可）。
...
| [docs/spike-findings.md](docs/spike-findings.md) | 真机实测的 45 条结论：透明合成、拖动、全屏、托盘、更新网络各一条 |
