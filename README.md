...
当前是 **1.5.0**：里程碑 1（核心摸鱼能力）已完成并通过真机验证。1.3.0 把起始页
...
- **背景透明度**另一条滑块（0%–100%）：只淡界面自己画的底板（顶栏、地址栏、右栏、
  ...
  点得到；网页与自家页面不受它影响
- **阅读透明度**第三条滑块（0%–100%）：只淡**正在读的正文**——本机 TXT 与自家 PDF
  阅读页，界面与网页都不受影响（见下面「离线阅读」一段）。前两条什么时候都管得着
...
- 功能收在右侧一条 48px 竖栏里：**收起时暂停播放**（全栏唯一一格开关，排在最上面）·
  站点 · 历史 · 书签 · 缩放 · **整体透明度 · 背景透明度 · 阅读透明度**。小窗口下这条栈
  是**会滚的**，排在下面的东西等于藏起来了——那一格开关因此排在最上面（三条滑块进来
  之后，960×540 这一档正好把它放满，迷你档 480×270 要滚 264px）
...
- 「离线阅读」那一栏里有一行**「打开文件…」**，按下去弹的是**系统的选文件框**；
  选中的 **TXT 与 PDF** 各开成一张标签页直接读——TXT 交给 Chromium 自己渲染，
  PDF 走这一页自己排（见下一条）
- **PDF 是自己排的**（pdf.js，Apache-2.0）：每一页画进一张画布，然后把**纸收掉、只留字**
  ——一张白纸在桌面上是逐像素透明的，字换成当前主题的墨色。同一本书在纸白主题下是
  近黑的字、暗夜主题下是近白的字。做这一条的起因是 Chromium 内置的阅读器改不动：
...
- **代价明写**：彩色会被放弃——插图与彩色标题按**深浅**变成一块墨，颜色没了。换来的
  是一份在任何主题下都读得下去的正文，这是这一版的取舍，不是漏掉了
...
- **正文可以单独淡下去**：右栏第三条滑块只作用在**被读的正文**上，界面与网页都不受
  影响，于是「界面 100% + 正文 40%」这种搭配成立。两种页各是一种实现——TXT 那一页是
  Chromium 自己渲染的，能碰的只有那一层，于是给 `html` 写一条 `opacity`；PDF 那一页是
  自家排的，直接给画布乘同一个数。**这一条永远不对网页生效**：网页不吃阅读透明度，
  这是硬边界（`isLocalFile` 一句话分界，见 [docs/architecture.md](docs/architecture.md)）
...
- **网页**各占一个视图，画在「顶栏之下、右栏之左」那个矩形里，界面与网页互不干扰；
  注入网页的透明样式只对访客页生效，碰不到自家页面；离线阅读的**正文透明度**走的也是
  这条路，但它**只给本机文件**（`file:`），网页那一侧永远吃不到——同一条注入通道，
  两个判据分明的对象
...
- **PDF 的排版还是固定的**：1.4.0 已经把底色与字色拿回来了（见「离线阅读」），
  字号、行距、页边距、单双页这些还没有——这一页现在是我们自己的了，接下来是在它上面加控件
...
| 文档 | 里面是什么 |
|---|---|
| [docs/architecture.md](docs/architecture.md) | 二十处与直觉相反的实现，改动前先读；另附目录与关键文件表 |
| [docs/verification.md](docs/verification.md) | `spike/` 下的探针：看不见屏幕时，怎么把一件件行为量出来 |
| [docs/spike-findings.md](docs/spike-findings.md) | 真机实测的 60 条结论：透明合成、拖动、全屏、托盘、更新网络与安装器、PDF 自渲染与键控、以及「注进去的样式撤不撤得掉」各一条 |
