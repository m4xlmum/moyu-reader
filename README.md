...
- 顶栏一条：前进 / 后退 / 刷新 / 站点标识 / 标签页 / 新标签，右侧依次是
  **最小化 · 关闭 · 悬浮球 · 收起右侧栏**
- 功能都收在右侧一条 48px 竖栏里：手机 / 置顶 / 迷你 / 站点 / 历史 / 书签 / 收藏 /
  缩放 / 透明度，设置固定在栏底
...
- 顶栏与右侧栏都能各自隐藏（设置 → 通用，或右键悬浮球）。顶栏藏起来后右栏会被保留：
  那是悬浮球此时唯一的落脚处

**隐藏策略：收起成悬浮球**
- 鼠标移出窗口后，**整个界面缩成一颗悬浮球**；点击球展开回原样
- 收起时窗口是真的缩到球的尺寸（28×28），不是把内容藏起来留一块空壳
- 因此屏幕上不存在「看不见却仍在接收点击」的区域——那类区域是最危险的失败模式，
  它会让本该落在 Word 里的点击被吞掉
- 球平时半透明、悬停变亮，**右键出菜单**：显隐顶栏 / 显隐右栏 / 藏进托盘 / 个人中心 / 退出
...
在**不显示的窗口**里单独渲染编译好的 chrome 界面（`spike/preview-preload.js`
是一个假的 `window.moyu`，主进程的窗口与标签页都不参与），截图并导出关键元素的
实测几何到 `spike/out/`。改版面时用它核对，不必一遍遍真的把应用窗口弹出来：
`--no-topbar` 看顶栏藏起来后球浮在右上角的样子，`--collapsed` 看收起态。
JSON 里的 `rightButtons` 就是顶栏右侧那一排按钮的顺序与坐标。
