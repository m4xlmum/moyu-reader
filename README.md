...
当前是 **1.5.2**：里程碑 1（核心摸鱼能力）已完成并通过真机验证。1.3.0 把起始页
...
- **正文可以单独淡下去**：右栏第三条滑块只作用在**正在读的那一份**上，界面与网页都不受
  ...
  自家排的，**淡的却是那张纸**（见上一条：位图里只剩墨，纸可以当一层元素底色补回来），
  于是拉下去是一张白纸渐渐铺回来，**字一个像素都不动**（1.5.2 改的）。**这一条永远
...
| [docs/spike-findings.md](docs/spike-findings.md) | 真机实测的 62 条结论：透明合成、拖动、全屏、托盘、更新网络与安装器、PDF 自渲染与键控、「注进去的样式撤不撤得掉」、阅读透明度为什么能淡纸而淡不到字，以及「几份文档之间到底有没有继承路径」（主题边界就建在它的答案上）各一条 |
