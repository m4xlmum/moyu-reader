...
当前是 **1.4.0**：里程碑 1（核心摸鱼能力）已完成并通过真机验证。1.3.0 把起始页
...
| 文档 | 里面是什么 |
|---|---|
| [docs/architecture.md](docs/architecture.md) | 十九处与直觉相反的实现，改动前先读；另附目录与关键文件表 |
| [docs/verification.md](docs/verification.md) | `spike/` 下的探针：看不见屏幕时，怎么把一件件行为量出来 |
| [docs/spike-findings.md](docs/spike-findings.md) | 真机实测的 57 条结论：透明合成、拖动、全屏、托盘、更新网络与安装器、PDF 自渲染与键控各一条 |
