...
当前是 **1.3.5**：里程碑 1（核心摸鱼能力）已完成并通过真机验证。1.3.0 把起始页
...
按身子回到它写着的那张网页，只有右端那枚箭头才展开清单；停在起始页 / 系统设置上时
它是回到网页的唯一入口，先前按它只会弹一份清单，还得再找一遍。
...
| [docs/spike-findings.md](docs/spike-findings.md) | 真机实测的 50 条结论：透明合成、拖动、全屏、托盘、更新网络与安装器各一条 |
